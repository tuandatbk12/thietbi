...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// CACHE HELPERS (REST-based, không cần supabase client)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
async function lookupCache(sbUrl: string, srvKey: string, hash: string): Promise<any> {
  try {
    const resp = await fetch(`${sbUrl}/rest/v1/rpc/lookup_chat_cache`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${srvKey}`,
        'apikey': srvKey,
    ...
    console.warn('[lookupCache]', e);
    return null;
  }
}

async function saveCache(
  sbUrl: string, srvKey: string,
  hash: string, question: string, answer: string, toolCalls: any[]
): Promise<void> {
  try {
    ...
    await fetch(`${sbUrl}/rest/v1/rpc/save_chat_cache`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${srvKey}`,
        'apikey': srvKey,
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const cached = await lookupCache(sbUrl, srvKey, qHash);
    if (cached) {
      console.log('[chat-query] Cache HIT (hits:', cached.out_hit_count, ')');
      
      await logChat({
        user_id: user.id,
        user_email: user.email,
        question,
        answer: cached.out_answer,
        tool_calls: cached.out_tool_calls ?? [],
        input_tokens: 0,
        output_tokens: 0,
      ...
      return new Response(JSON.stringify({
        answer: cached.out_answer,
        toolCalls: cached.out_tool_calls ?? [],
        usage: { input_tokens: 0, output_tokens: 0, duration_ms: Date.now() - t0 },
        from_cache: true,
        cached_at: cached.out_cached_at,
        cache_hits: cached.out_hit_count,
    ...
    // ━━ SAVE TO CACHE ━━
    try {
      await saveCache(sbUrl, srvKey, qHash, question, finalAnswer, toolCalls);
