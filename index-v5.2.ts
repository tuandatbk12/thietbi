// ════════════════════════════════════════════════════════════════
// Edge Function v5.2 — Better 429 handling + retry-in respect
// 
// Changes vs v5.1:
//   - Retry on 429 tăng từ 3 → 6 lần
//   - Đọc "Please retry in Xs" từ error message → đợi đúng X giây
//   - Fallback backoff 30s nếu không parse được
// ════════════════════════════════════════════════════════════════
...
11. ngay_kiem_dinh: YYYY-MM-DD - lấy từ "Ngày kiểm định" HOẶC "Ngày thí nghiệm"
...
16. sfra: CHỈ cho MBA. Tìm "Đo đáp ứng tần số quét" → có X = true, trống = false. Loại khác: null
17. tiet_dien: CHỈ cho Cáp (format "1x630 mm²" / "3x50 mm²"). Loại khác: null

Trả CHỈ JSON, KHÔNG markdown. Trường không đọc được = null.`;
...
// ────────────────────────────────────────────────────────────
// Better 429 handling: parse "retry in Xs" from error message
// ────────────────────────────────────────────────────────────
function _parseRetryInSeconds(errText: string): number {
  // "Please retry in 54.840120488s."
  const m = errText.match(/retry in (\d+(?:\.\d+)?)s/i);
  if (m) return Math.ceil(parseFloat(m[1])) + 1; // +1 buffer
  return 30; // fallback
}
...
async function callGeminiOcr(parts: any[]): Promise<{ text: string; elapsed: number; retries: number }> {
  ...
  while (true) {
    ...
    // Parse retry-in for 429
    let waitMs = 0;
    if (geminiRes.status === 429) {
      const errText = await geminiRes.clone().text();
      const waitS = _parseRetryInSeconds(errText);
      waitMs = waitS * 1000;
      console.log(`[Gemini] 429 quota, wait ${waitS}s (retry ${retryCount}/${MAX_RETRIES})`);
    } else {
      waitMs = 2000 * retryCount;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`[OCR] Gemini OK, ${elapsed}ms, retries: ${retries}`);
